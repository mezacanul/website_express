...
var batchItem = (el) => { 
    return `
        <div class="item" data-sku="${el.sku}" data-priceId="${el.priceId}">
            <div class="itemFlexHalf">
                <p class="itemName" onclick="location.assign('preview.html?sku=${el.sku}')">${el.name}</p>
                <p class="itemPrice">$${parseFloat(el.price)}</p>
                <button onclick="switchThis(this)" type="button">Switch</button>
            </div>
            <div class="itemFlexHalf">
                <img src="files/img/products/${el.fileId}">
            </div>
